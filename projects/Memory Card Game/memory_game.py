...
#initialization pygame for sound
pygame.mixer.init()
...
# flip_sound = load_sound(r"C:\Users\DELL\Desktop\PYTHON WORKSHOP\DAY7\Memory Card Game\flip.wav")
...
def load_sound(file_path, duration=2):
    try:
        ...
        print(f"Warning: Sound file {file_path} not found. Continuing without sound.")
        # Return a dummy sound object that does nothing
        class DummySound:
            def play(self): pass
        return DummySound()

# Load sounds (will work even if files are missing)
flip_sound = load_sound("flip.wav")
match_sound = load_sound("match.wav")
win_sound = load_sound("win.wav")
# flask api url
...
root = tk.Tk()
root.title("Two_player Memory Game")
root.geometry("800x600")#initialize window size
...
#ADD A title
title_label = tk.Label(main_frame,text="Memory Card Game", font=TITLE_FONT,fg=TEXT_COLOR,bg=BG_COLOR)
title_label.pack(pady=10)

# CREATE A CANVVAS FOR THE CARD 
...
# load card images(replace with your image path)
card_images = [
    resize_image("card1.jpg", 100, 100),
    resize_image("card2.jpg", 100, 100),
    resize_image("card3.jpg", 100, 100),
    resize_image("card4.jpg", 100, 100),
    resize_image("card5.jpg", 100, 100),
    resize_image("card6.jpg", 100, 100)
]
...
card_back = resize_image("card_back.jpg", 100, 100)
...
#flip a card
def flip_card(index):
    if len(flipped) < 2 and not cards[index]["matched"]:
...
#     canvas.delete("score")
#     canvas_width = canvas.winfo_width()
#     # canvas.create_text(canvas_width)// 2, 20, text=f"{player_names[1]}:{scores[1]} | {player_names[2]}:{scores[2]} | Turns: {turns[1]} vs {turns[2]} | Time: {int(time.time () - start_time)}s | Current Turn: {player_names[current_player]}", font=FONT, fill=TEXT_COLOR, tag="score")
#     canvas.create_text(canvas_width // 2, 20, text=f"{player_names[1]}:{scores[1]} | {player_names[2]}:{scores[2]} | Turns: {turns[1]} vs {turns[2]} | Time: {int(time.time() - start_time)}s | Current Turn: {player_names[current_player]}", font=FONT, fill=TEXT_COLOR, tag="score")

def update_score():
    canvas.delete("score")  # Ensure 'score' tag exists before deleting
    canvas_width = canvas.winfo_width()
    canvas.create_text(
        canvas_width // 2, 20,
        text=f"{player_names[1]}: {scores[1]} | {player_names[2]}: {scores[2]} | "
             f"Turns: {turns[1]} vs {turns[2]} | Time: {int(time.time() - start_time)}s | "
             f"Current Turn: {player_names[current_player]}",
        font=FONT,
        fill=TEXT_COLOR,
        tags="score"  # Use `tags` instead of `tag`
    )
...
def save_scores(game_time):
    for player_id, scores in scores.items():
        # requests.post(f"{API_URL}/save_score", json={"palyer_id": player_id,"score": score, "time":game_time})
        requests.post(f"{API_URL}/save_score", json={"player_id": player_id, "score": scores, "time": game_time})

def show_leaderboard():
    try:
        response = requests.get(f"{API_URL}/leaderboards")
        ...
        if leaderboard:
            for entry in leaderboard:
                tk.Label(leaderboard_window, text=f"{entry[0]}: {entry[1]} points in {entry[2]}seconds", font=FONT, bg=BG_COLOR, fg=TEXT_COLOR).pack()
        else:
            tk.Lable(leaderboard_window, text="No data available", font=FONT, bg=BG_COLOR, fg=TEXT_COLOR).pack()
...
def get_player_names():
    player_names[1] = simpledialog.askstring("Player 1", "Enter Player 1 name", parent=root)
    player_names[2] = simpledialog.askstring("Player 2", "Enter Player 2 name", parent=root)
...
def create_button(text, command):
    # button = tk.Button(main_frame, text=text font=BUTTON_FONT,bg=BUTTON_COLOR,fg=TEXT_COLOR, activebackground=HOVER_COLOR, command=command)
    button = tk.Button(main_frame, text=text, font=BUTTON_FONT, bg=BUTTON_COLOR, fg=TEXT_COLOR, activebackground=HOVER_COLOR, command=command)
